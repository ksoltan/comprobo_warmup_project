...
class EstopBehavior(object):
    def __init__(self):
        ...
        self.cmd_vel_publisher = rospy.Publisher("/desired_cmd_vel", DesiredVelocity, queue_size=10, latch=True)
    ...
    def estop(self, bump_msg):
        ...
        if(bump_msg.leftSide or bump_msg.leftFront or bump_msg.rightSide or bump_msg.rightFront):
            ...
            self.cmd_vel_publisher.publish(DesiredVelocity(node_ID=str(State.ESTOP), velocity=PolarVelocity2D(linear=0, angular=0)))
